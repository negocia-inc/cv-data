...
FROM `ida-prd.ipalette_optout.line_ad_image_assets` AS a
JOIN `ida-prd.ipalette_optout.line_ad_uploaded` AS b
ON a.ad_account_id = b.ad_account_id
AND a.campaign_id = b.campaign_id
AND a.ad_group_id = b.ad_group_id
AND a.ad_id = b.id
JOIN `ida-prd.ipalette_optout.line_ad` as c
...
AND b.type in ('IMAGE', 'STATIC BANNER')
GROUP BY
  campaign_id,
  ad_group_id,
  id,
