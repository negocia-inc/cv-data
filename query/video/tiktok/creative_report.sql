...
FROM `ida-prd.ipalette_optout.tiktok_ad_video_assets` AS a
JOIN `ida-prd.ipalette_optout.tiktok_ad_uploaded` AS b
ON a.ad_account_id = b.ad_account_id
AND a.campaign_id = b.campaign_id
AND a.ad_group_id = b.ad_group_id
AND a.ad_id = b.id
JOIN `ida-prd.ipalette_optout.tiktok_ad` as c
ON a.ad_account_id = c.ad_account_id
AND a.campaign_id = c.campaign_id
AND a.ad_group_id = c.ad_group_id
AND a.ad_id = c.id
WHERE date between '2021-10-01' and '2024-09-30'
AND b.type="SINGLE_VIDEO"
GROUP BY
  campaign_id,
  ad_group_id,
  id,
